...
const SignUpForm = ({ setTypeSelected }: AuthFormProps) => {
  ...
  const onSubmit = async (user: z.infer<typeof formSchema>) => {
    console.log('🔴 onSubmit llamado con:', user);
    setisLoading(true);

    try {
      console.log('🔵 Llamando a signup...');
      const res = await signup(user);
      console.log('🟢 Respuesta de signup:', res);

      if (!res.success) {
        console.log('❌ Signup falló:', res.message);
        toast.error(res.message || 'Error al registrar el usuario', {
          duration: 4000,
        });
        return;
      }
      toast.success(`Bienvenido ${user.name}!`, { duration: 2500 });
      window.location.href = '/dashboard';

      console.log('✅ Signup exitoso');
      toast.success(
        `Hola ${user.name}, te hemos enviado un correo para validar tu cuenta.`,
        { duration: 4000 }
      );
